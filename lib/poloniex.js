...
function Poloniex (options) {
    ...
    self.getTrades = function (options, callback) {
        ...
        poloniexPrivate.returnMyTradeHistory(currencyPair, null, null, function (err, xResult) {
            let result = {
                timestamp: util.timestampNow(),
                error: "",
                data: []
            ...
            _.forEach(xResult, (order, index) => {
                let newTrade = {
                    trade_id: order.orderNumber,
                    type: order.type,
                    amount: order.amount,
                    price: order.rate,
                    timestamp: util.timestamp(moment.utc(order.date).format('X')),
                };
    ...
    self.getFee = function (options, callback) {
        ...
        poloniexPrivate.returnFeeInfo(function (err, poloniexFee) {
            ...
            let data = {
                ...
                maker_fee: poloniexFee.makerFee.toString(),
                taker_fee: poloniexFee.takerFee.toString(),
            };
            fee.data.push(data);
            callback(null, fee);
        });
...
Poloniex.prototype.properties = {
    ...
    methods: {
        implemented: ['getRate', 'getTicker', 'getOrderBook', 'getBalance', 'getMarginPositions', 'postSellOrder', 'postBuyOrder', 'getOpenOrders', 'getFee', 'getTrades'],
        notSupported: [],
    },
